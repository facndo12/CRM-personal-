...
export class WhatsAppManager {
  ...
  async connect(workspaceId: string, options?: ConnectOptions) {
    ...
    if (!authAvailable && mode === 'pairing' && !normalizedPhone) {
      throw new ValidationError('La primera vinculacion requiere un numero con codigo de pais para generar el pairing code.')
    }
    ...
    if (mode === 'pairing' && normalizedPhone && !entry.state?.creds?.registered) {
      await sleep(1200)
      const pairingCode = await entry.sock.requestPairingCode(normalizedPhone)

      await this.updateSession(workspaceId, {
        status: 'PAIRING',
        phoneNumber: normalizedPhone,
        pairingCode,
        pairingCodeIssuedAt: new Date(),
        qrCode: null,
        lastError: null,
      })
    }

    if (mode === 'qr' && !entry.state?.creds?.registered) {
      await this.updateSession(workspaceId, {
        status: 'PAIRING',
        phoneNumber: null,
        pairingCode: null,
